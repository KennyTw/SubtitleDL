...
const __getTitle = full => {
  ...
  if(!full) {
    const episodeElementTitle = titleElement.nextElementSibling;
    if(episodeElementTitle) {
    const m = episodeElementTitle.textContent.replace("：","");
...
const processSubInfo = async result => {
  ...
  for(const track of tracks) {
    ...
    if(track.language != 'en' && track.language != 'zh-Hant')
      continue;
