...
for zipfile in glob.glob("./subtitle/*.zip"):
	ZipTargetPathName = Path(zipfile).stem
	with ZipFile(zipfile, 'r') as zipObj:
		ZipTargetPath = "./" + str(Path(zipfile).parent) + "/" + ZipTargetPathName + "/"
		print(ZipTargetPath)
		zipObj.extractall(path=ZipTargetPath)
		for en in glob.glob(ZipTargetPath + "*.en[[]cc[]].vtt"):
			print(en)
			enFileName = en
		for cht in glob.glob(ZipTargetPath + "*.zh-Hant.vtt"):
			print(cht)
			chtFileName = cht
		if enFileName != "" and chtFileName != "":
			os.remove(zipfile)
...
if enFileName != "" and chtFileName != "":
	...
	with open(ZipTargetPath + 'ouput.txt','w') as data:
